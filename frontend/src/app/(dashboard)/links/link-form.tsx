...
export function LinkForm({
  ...
  const devices = new Set(
    link?.targetingRules?.devices ??
      (isCreate ? [...LINK_DEFAULTS.targetingDevices] : []),
  );
          ...
          <section className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="iOS App Store URL" htmlFor="iosAppStoreUrl">
              <Input
                ...
                defaultValue={
                  link?.iosAppStoreUrl ??
                  (isCreate ? LINK_DEFAULTS.iosAppStoreUrl : "")
                }
              />
            </Field>
            <Field label="Android store URL" htmlFor="androidAppStoreUrl">
              <Input
                ...
                type="url"
                defaultValue={
                  link?.androidAppStoreUrl ??
                  (isCreate ? LINK_DEFAULTS.androidAppStoreUrl : "")
                }
              />
            </Field>
              ...
              [
                [
                  ...
                  link?.utmParameters?.source ??
                    (isCreate ? LINK_DEFAULTS.utmParameters.source : undefined),
                ],
                [
                  "utmMedium",
                  "Medium",
                  link?.utmParameters?.medium ??
                    (isCreate ? LINK_DEFAULTS.utmParameters.medium : undefined),
                ],
                [
                  "utmCampaign",
                  "Campaign",
                  link?.utmParameters?.campaign ??
                    (isCreate ? LINK_DEFAULTS.utmParameters.campaign : undefined),
                ],
